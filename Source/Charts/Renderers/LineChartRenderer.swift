...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    open func drawLinearFill(context: CGContext, dataSet: ILineChartDataSet, trans: Transformer, bounds: XBounds)
    {
		...
		if let fill2 = dataSet.secondaryFill {
			 drawFilledPath(context: context, path: filled, fill: fill2, fillAlpha: dataSet.fillAlpha)
		}
    }
                ...
                for j in _xBounds
                {
                    ...
                    if dataSet.isDrawValuesEnabled {
                        ...
                        ChartUtils.drawText(
                            ...
                            point: CGPoint(
                                x: pt.x + valueOffsetFromIcon.x,
                                y: pt.y - CGFloat(valOffset) + valueOffsetFromIcon.y - valueFont.lineHeight),
                            align: .center,
